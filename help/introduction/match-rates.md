...
La synchronisation des identifiants correspond aux identifiants attribués par le service d’ID aux identifiants attribués aux visiteurs du site par nos clients. Par exemple, supposons que le service d’ID ait attribué un ID de visiteur 1234. Une autre plate-forme connaît ce visiteur par ID 4321. Le service d’ID associe ces identifiants lors du processus de synchronisation. Les résultats ajoutent de nouveaux points de données à ce que nos clients savent sur leurs visiteurs de site. Et si le service d’ID ne peut pas correspondre à un ID, il en crée un nouveau et l’utilise pour une synchronisation ultérieure.
...
**Étape 3 : Synchronisation des identifiants d’incendie**
