...
# Mettre en œuvre le service d’identification des visiteurs Adobe pour Analytics, Audience Manager et Target {#implement-the-experience-cloud-id-service-for-analytics-audience-manager-and-target}

Ces instructions s’adressent aux clients Analytics, Audience Manager et Target qui souhaitent utiliser le service d’identification des visiteurs et n’utilisent pas de [balises](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=fr). Cependant, nous vous recommandons vivement d’utiliser les balises pour implémenter le service d’identification des visiteurs. Les balises optimisent le workflow d’implémentation et assurent automatiquement le placement et le séquencement adéquats du code.

>[!IMPORTANT]
>
>Lisez la section Service d’identification des visiteurs [conditions requises](../reference/requirements.md) avant de commencer et notez les conditions requises suivantes qui sont spécifiques à cette implémentation :
>
...
1. Accédez à **[!UICONTROL Admin > Code Manager]**.
...
Si l’un de ces cas d’utilisation s’applique à votre situation, demandez à l’[Assistance clientèle](https://helpx.adobe.com/fr/marketing-cloud/contact-support.html) de configurer une période de grâce temporaire. Les périodes de grâce peuvent durer jusqu’à 180 jours. Vous pouvez renouveler une période de grâce si nécessaire.
...
Les solutions Entreprise CX de cette implémentation renvoient des identifiants sous la forme de paires clé-valeur. Chaque solution utilise différentes clés (par exemple, le SDID Analytics ou le mboxMCSDID Target) pour contenir le même identifiant. Pour tester votre mise en œuvre, chargez votre page dans un environnement de développement. Utilisez votre console ou logiciel de navigation qui surveille les demandes et réponses HTTP pour vérifier les identifiants répertoriés ci-dessous. Le service d’identification des visiteurs a été correctement mis en œuvre lorsque les paires clé-valeur répertoriées ci-dessous renvoient les mêmes valeurs d’identifiant.
...
* ECID avec le débogueur Adobe ou votre outil de débogage préféré.

Pour des vérifications supplémentaires qui vous aident à déterminer si le service d’identification des visiteurs fonctionne correctement, voir [Test et vérification du service d’identification des visiteurs](../implementation-guides/test-verify.md).
...
Lorsque vous voyez un mboxMCAVID, vérifiez sa valeur par rapport à l’outil Analytics AID. Ces valeurs sont identiques lorsque le service d’identification des visiteurs a été correctement implémenté.
